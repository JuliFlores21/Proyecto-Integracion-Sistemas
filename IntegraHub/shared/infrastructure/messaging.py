...
class RabbitMQConnection:
    def __init__(self, host: str, user: str = None, password: str = None):
        self.host = host
        self.user = user or os.getenv("RABBITMQ_USER", "user")
...
class BasePublisher:
    def __init__(
        self, connection: RabbitMQConnection, exchange_name: str = "integrahub_exchange"
    ):
    ...
    def publish(
        self, topic: str, event_type: str, data: dict, correlation_id: str = None
    ):
...
class BaseConsumer:
    ...
    def __init__(
        ...
        service_name: str,
        exchange_name: str = "integrahub_exchange",
    ):
        self.connection_wrapper = connection
    ...
    def setup_topology(self):
        ...
        # 2. DLX setup
        channel.exchange_declare(
            exchange=self.dlx_name, exchange_type="direct", durable=True
        )
        channel.queue_declare(queue=self.dlq_name, durable=True)
        channel.queue_bind(
            queue=self.dlq_name,
            exchange=self.dlx_name,
            routing_key=f"{self.service_name}_dlq_key",
        )
    ...
    def start_consuming(self, callback_function: Callable):
        ...
        def wrapper_callback(ch, method, properties, body):
            print(
                f" [x] Received {method.routing_key} | CorrId: {properties.correlation_id}"
            )
            try:
        ...
        channel.basic_consume(
            queue=self.queue_name, on_message_callback=wrapper_callback
        )
